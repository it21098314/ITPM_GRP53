...
function ProductPage() {
    ...
    const handleEnroll = () => {
        console.log('Enroll');
        setIsEnrolled(true); // update state on enrollment
      };

      const isSuccesss = isEnrolled;
  ...
  return (
    ...
    <Row>
        ...
        <Col lg={6} className="pt-4">
                    ...
                    <b><p className="product__price">${product.price}</p></b>
                    <p style={{ textAlign: "justify" }} className="py-3">
                        <strong>Description:</strong> {product.description}
                    </p>

          {user && !user.isAdmin && product.category === 'free' && (
                ...
                <option value="5">5</option>
                    ...
                    {user && user.isAdmin && (
                        <LinkContainer to={`/product/${product._id}/edit`}>
                            <Button size="lg">Edit Course</Button>
                        </LinkContainer>
                    )}
                      {isSuccess && <ToastMessage bg="info" title="Added to cart" body={`${product.name} is in your cart`} />}
                      {isSuccesss && <ToastMessage bg="info" title="Enrollment Successful" body="You have successfully enrolled Course access will recived to your email..." />}
                </Col>   
